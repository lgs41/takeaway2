...
<head>
    ...
    <script>
        ...
        function toFoodAddBatch(){
            ...
            window.location.href="${pageContext.request.contextPath}/buy/toAddBatch?userId=${sessionScope.userInfo.id}"+"&"+ids;
        ...
        function toFoodDelBatch(){
            ...
            window.location.href="${pageContext.request.contextPath}/buy/toDelBatch?"+ids;
        }
...
<div style="margin-left: 300px;margin-top: 80px">
    请输入要查询的商品名：
    <form action="${pageContext.request.contextPath}/buy/toBuyInfo" method="post">
        <input name="foodName" value="${foodName}"> <input type="submit" value="搜索">
    </form>
    <a href="javascript:toFoodAddBatch()" target="main">下单</a>
    <a href="javascript:toFoodDelBatch()" target="main">删除</a>
</div>
...
<table border="1" cellspacing="0" width="1000px" style="margin-top: 20px;margin-left: 5px">
    ...
    <c:forEach items="${requestScope.pageInfo.list}" var="temp">
        <tr>
            ...
            <td style="text-align: center">${temp.addData}</td>
        </tr>
    </c:forEach>
</table>
...
<div style="margin-left: 450px">
    <c:if test="${pageInfo.hasPreviousPage}">
        <a href="${pageContext.request.contextPath}/buy/toBuyInfo?foodName=${foodName}">首页</a>
        <a href="${pageContext.request.contextPath}/buy/toBuyInfo?pageNum=${pageInfo.pageNum-1}&foodName=${foodName}">上一页</a>
    </c:if>

    <c:forEach begin="${begin}" end="${end}" var="i">
        <a href="${pageContext.request.contextPath}/buy/toBuyInfo?pageNum=${i}&foodName=${foodName}">${i}</a>
    </c:forEach>

    <c:if test="${pageInfo.hasNextPage}">
        <a href="${pageContext.request.contextPath}/buy/toBuyInfo?pageNum=${pageInfo.pageNum+1}&foodName=${foodName}">下一页</a>
        <a href="${pageContext.request.contextPath}/buy/toBuyInfo?pageNum=${pageInfo.pages}&foodName=${foodName}">尾页</a>
    </c:if>
</div>
