...
<head>
    ...
    <script>
        function toFoodEdit(orderId,state){
            if(state=="已评价"){
                alert("该订单已送达，请勿重复操作");
                return;
            }
            ...
            window.location.href="${pageContext.request.contextPath}/order/toFoodEdit?orderId="+orderId
        }
        function toDelBatch(){
            //获取选中的记录
...
<div style="margin-left: 300px;margin-top: 80px">
    请输入要查询的商品名称：
    <form action="${pageContext.request.contextPath}/order/toOrderInfo" method="post">
    ...
    <c:forEach items="${requestScope.pageInfo.list}" var="temp">
        <tr>
            ...
            <td style="text-align: center">
                <a href="javascript:toFoodEdit('${temp.orderId}','${temp.state}')">已送达</a>
            </td>
        </tr>
    </c:forEach>
</table>
...
<div style="margin-left: 450px">
    <c:if test="${pageInfo.hasPreviousPage}">
        <a href="${pageContext.request.contextPath}/order/toOrderInfo?foodName=${foodName}">首页</a>
        <a href="${pageContext.request.contextPath}/order/toOrderInfo?pageNum=${pageInfo.pageNum-1}&foodName=${foodName}">上一页</a>
    </c:if>

    <c:forEach begin="${begin}" end="${end}" var="i">
        <a href="${pageContext.request.contextPath}/order/toOrderInfo?pageNum=${i}&foodName=${foodName}">${i}</a>
    </c:forEach>

    <c:if test="${pageInfo.hasNextPage}">
        <a href="${pageContext.request.contextPath}/order/toOrderInfo?pageNum=${pageInfo.pageNum+1}&foodName=${foodName}">下一页</a>
        <a href="${pageContext.request.contextPath}/order/toOrderInfo?pageNum=${pageInfo.pages}&foodName=${foodName}">尾页</a>
    </c:if>
</div>
